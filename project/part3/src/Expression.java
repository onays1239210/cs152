...
class BinOpExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
    	...
    	IntVal a = null;
    	IntVal b = null;
    		if(e2.evaluate(env).getClass() != e1.evaluate(env).getClass()){
			
			if (e2.evaluate(env).toString()=="null") {
				b = (IntVal) (new ValueExpr(new IntVal(0))).evaluate(env);
			}
			
			if (e1.evaluate(env).toString()=="null") {
				a = (IntVal) (new ValueExpr(new IntVal(0))).evaluate(env);
			}
			
			
			if (e2.evaluate(env).toString().contains("function")) {
				b = (IntVal) (new ValueExpr(new IntVal(1))).evaluate(env);
			}
			
			if (e1.evaluate(env).toString().contains("function")) {
				a = (IntVal) (new ValueExpr(new IntVal(1))).evaluate(env);
			}
			
    	}
		else {
			b = (IntVal) e2.evaluate(env);
			a = (IntVal) e1.evaluate(env);
    	}
    	...
    	switch(op){
    	case ADD:
    		return (new IntVal(a.toInt() + b.toInt()));
    	case SUBTRACT:
    		return (new IntVal(a.toInt() - b.toInt()));
    	case MULTIPLY:
    		return (new IntVal(a.toInt() * b.toInt()));
...
class WhileExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
        ...
        while(bool.toBoolean()){
        	body.evaluate(env);
        	bool = (BoolVal)cond.evaluate(env);
        }
        return null;
    }
}
...
class FunctionDeclExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
    	//System.out.println("Delcar  "+params+"   "+body);
    	ClosureVal temp = new ClosureVal(params, body, env);
        return temp;
    }
}
...
class FunctionAppExpr implements Expression {
    ...
    public Value evaluate(Environment env) {
    	...
    	ClosureVal temp = (ClosureVal)f.evaluate(env);
    	for(Expression e: args){
    		      ValArgs.add(e.evaluate(env));
    	}
    	
        return temp.apply(ValArgs);
    }
}
