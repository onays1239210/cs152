...
route(Departing,Arriving,Visited,DepartureTime,ArrivalTime,Flights,TotalCost) :-
	...
	not(member(Departing,Visited)),
	Flights = [Departing, Arriving].

route(Departing, Arriving, Visited, DepartureTime, ArrivalTime, Flights, TotalCost) :-
    flight(Departing, X,DepartureTime,H2:M2, Inicost),
    not(member(X, Visited)),   %doesn't visited X before
    X \= Arriving,
    route(X, Arriving, [Departing|Visited], SubDepartureTime, SubArrivalTime,TailFlights, Restcost),
    H1:M1 = SubDepartureTime,  %get hours and mins
      (
        H1 > H2		       %compare hours
        ;
        H1 = H2,                  %if hours are the same
        M1 > M2                   %compare mins
      ),
    ArrivalTime = SubArrivalTime,
    Flights = [Departing|TailFlights],
    TotalCost is Inicost + Restcost.
