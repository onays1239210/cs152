...
(define (big-add1 x y co)
  (cond
    ...
    [else
      (if (< 1000 (+ (car x) (car y)))
          (append (list (modulo (+ (car x) (+ co (car y))) 1000)) (big-add1 (cdr x) (cdr y) (quotient (+ (car x) (car y)) 1000)))
          (append (list (+ (car x) (+ co (car y)))) (big-add1 (cdr x) (cdr y) 0))) 
...
(define (big-subtract1 x y borrow)
  ( cond [(and (= 0 (length x)) (= 0 (length y))) '()]
         [(= 0 (length y))  (cons (- (car x) borrow) (big-subtract1 (cdr x) '() 0))]
         [(> (car x) (car y)) (cons (- (car x) (+ (car y) borrow)) (big-subtract1 (cdr x) (cdr y) 0))]
         [(< (car x) (car y))
          (if (null? (cdr x))
              (error "negative number")
              (cons (- (+ 1000 (car x)) (+ (car y) borrow)) (big-subtract1 (cdr x) (cdr y) 1)))]
         [(= (car x) (car y)) (cons 0 (big-subtract1 (cdr x) (cdr y) 0))]))


;; Returns true if two big-nums are equal
(define (big-eq x y)
  (cond [(and (empty? x) (empty? y)) "equal"]
        [(= (length x) (length y))
         (if (= (car x) (car y))
             (big-eq (cdr x) (cdr y))
             ("not equal"))]
...
(define (big-multiply x y)
 (cond [(empty? x) '()]
       [(< 1 (car y)) (big-add x (big-multiply x (big-dec y)))]
       [else x]))

;; Raise x to the power of y
(define (big-power-of x y)
   (cond [(empty? x) '()]
       [(< 1 (car y)) (big-multiply x (big-power-of x (big-dec y)))]
       [(= 1 (car y)) x]
       [(= 0 (car y)) '(1)]))
